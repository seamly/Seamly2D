...
- Local debug build: `scripts/sd.ps1` — auto-detects the newest Qt 6.10.x msvc2022_64 kit under `C:\Qt` and the VS 18 MSVC environment, shadow-builds `CONFIG+=debug` into `scripts/seamly2d-build-debug/` (gitignored); debug exe at `scripts/seamly2d-build-debug/src/app/seamly2d/bin/seamly2d.exe`, Qt debug DLLs deployed by windeployqt; `-Run` launches after build.
- seamlyLayout builds separately with `src/app/seamlylayout/qd.ps1` and must stay out of the qmake build.

## Continuous integration (CI)

Three independent GitHub Actions workflows, split by Qt toolchain / purpose (see the `README_WORKFLOWS.md` in `.github/workflows/` for the full descriptions):
...
| `windows-msi.yml` (**Task 13**) | seamly2d, seamlyme, seamlyLayout | 6.11.1 (parents) + 6.10.1 (layout) | windows-2022 | builds the parents (qmake) and SeamlyLayout (CMake) in one job, then the bundled WiX **.msi** per arch (x64 = all three, arm64 = parents only), validates, signs, uploads the MSI artifact |

- **Why split workflows:** seamly2d/seamlyme use the qmake toolchain the GitHub release runners provide (Qt 6.11.1); seamlyLayout requires **Qt 6.10.1** plus a Rust + CMake/Ninja toolchain (`find_package(Qt6 6.10.1 REQUIRED ...)` in `qt_frontend/CMakeLists.txt`). Keeping `ci.yml` and `seamlylayout-ci.yml` separate keeps them fully independent — a seamlyLayout failure never blocks the seamly2d/seamlyme jobs, and vice versa. `windows-msi.yml` is the one workflow that deliberately combines **both** toolchains in a single job (it needs to build all three apps to bundle them), which is exactly why it is kept out of `ci.yml` — it must not slow down or destabilize every push.
...
- **MSI installer (Task 13) — shipped:** a single bundled Windows **.msi** per architecture (x64 and arm64) that installs all three apps together, built with the **WiX toolset** from `scripts/packaging/windows/seamly-family.wxs` via `scripts/smsi.ps1` (local) and `.github/workflows/windows-msi.yml` (CI). The hands-on build/test reference is `scripts/packaging/windows/README.md`. Key design points:
  - **WiX v6, not v7.** WiX v7 (July 2026) refuses to run until an Open Source Maintenance Fee (OSMF) EULA is accepted (error `WIX7015`) — a policy decision the project has not made — so both the script and the workflow pin the `wix` .NET tool to `6.*`. The `WixToolset.UI.wixext` version must match the installed core tool.
  - **One bundled MSI per arch, not per-app MSIs** — the three apps are a family and are installed/removed together.
  - **Install layout.** `\Program Files\Seamly2D\` holds `seamly2d.exe` + `seamlyme.exe` and their **shared** Qt runtime (windeployqt output + xerces-c + app-local MSVC CRT). `\Program Files\Seamly2D\SeamlyLayout\` holds `SeamlyLayout.exe` with its **own** Qt runtime — SeamlyLayout builds against a different Qt release (6.10) than the parent apps, and the Qt DLL file names are identical across releases, so the two runtimes cannot share a flat directory. seamly2d finds the subdirectory executable at runtime via `SeamlyFamilyPaths::locateSeamlyLayout()` (`src/libs/vmisc/seamly_family_paths.cpp`), which checks the flat-beside-parent layout first, then the `SeamlyLayout\` subdirectory.
  - **MSVC CRT deployed app-locally** (`msvcp140.dll`, `vcruntime140*.dll`, ...) beside each executable — no merge modules, no `vc_redist.exe` chaining. `smsi.ps1` finds the CRT via `VCToolsRedistDir` (falling back to scanning VS installs).
  ...
  - **arm64.** seamly2d/seamlyme cross-compile for arm64 exactly as in `ci.yml`'s windows matrix. **SeamlyLayout has no arm64 build yet** (its Rust + cxx-qt + Qt 6.10 cross story is unresolved), so the arm64 MSI ships the two parent apps only — `smsi.ps1 -NoSeamlyLayout`. When SeamlyLayout gains an arm64 build, drop that flag.
