...
## Task 27 — Fix the 7 pre-existing seamlylayout Rust test failures (layout_tiling, polygon_pack)

`cargo test --workspace` in `src/app/seamlylayout` fails 7 of 251 tests. Discovered during the Task 19 move verification (2026-07-19); the crates are byte-identical to the nested repo's `main`, so the failures predate the move — they look like real logic/test drift, not relocation breakage. The failing tests:

**`layout_tiling` (3):**

1. `layout_settings::tests::selvedge_deduction` (`layout_settings.rs:504`) — expects `effective_bin_px()` to deduct the selvedge from both sides (`(36 − 2×0.5) × 96 = 3360`) but gets `3456` (= 36 × 96, no deduction). Note the test fixture uses `media_type: "paper"` — decide whether selvedge should apply to paper at all (it is a fabric concept) or whether the test fixture should use fabric; fix code or test accordingly
2. `tiling::tests::tiled_svg_col_resets_per_row` (`tiling.rs:1051`) — row 2's path data should start at `minX=24`, got `M 24,524 M 524,524`
3. `tiling::tests::tiled_svg_path_count` (`tiling.rs:916`) — expects 3 path elements, got 4

**`polygon_pack` (4):**

4. `placer::tests::place_orders_by_area_descending` (`placer.rs:1027`)
5. `placer::tests::place_two_squares_pack_flush` (`placer.rs:1001`)
6. `tests::pack_polygons_axis_aligned_pieces_pick_zero_rotation` (`lib.rs:285`)
7. `tests::pack_polygons_respects_gap_px` (`lib.rs:312`)

- [ ] Use the archived nested-repo history (`C:\Users\susan\Projects\seamlyLayout-dotgit-archive`, or `github.com/seamly/seamlyLayout`) to bisect/date when each test started failing and whether the code or the test drifted
- [ ] `layout_tiling`: settle the selvedge-for-paper question and fix `effective_bin_px()` or the test; fix the tiled-SVG row-reset and path-count behavior (or update the tests if the current output is the intended contract)
- [ ] `polygon_pack`: diagnose the 4 placer/packing failures (ordering by area, flush packing, zero-rotation preference, gap respect) — these are core nesting behaviors, so treat code regressions as the default assumption, verify layouts visually with the richmond test pattern if needed
