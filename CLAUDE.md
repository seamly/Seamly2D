...
- **File naming:** snake_case, all lowercase; name files for what they do, using the prefixes the style guide lists (`settings_*`, `dialog_<toolgroup>_<toolname>`, `tool_*`, `model_*`, `options_*`, `test_*`, `application_<appname>`, …). No abbreviations, no generic names (`util.h`, `helpers.cpp`). **Unique repo-wide: a search for `<filename.extension>` must return exactly one file.** Do NOT start a file name with a bare `s` — that older anti-`v` convention is superseded by the prefix list. New source files still must not begin with `v`; existing `v*` files keep their names when edited unless a task renames them
- **Class naming:** UpperCamelCase (the project's deliberate deviation from JSF-AV), and a file that primarily defines one class is named after it — `settings_common.cpp` ↔ `SettingsCommon`
