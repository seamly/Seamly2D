...
Supporting workflow config, so a skip is the only thing left to decide: all three workflows carry `concurrency` with `cancel-in-progress: true` (several merges in one session collapse to one run instead of queueing N), and `ci.yml`'s push trigger has a `paths-ignore` for `**.md`, `project-docs/**`, `LICENSE`, `.claude/**` and `.vscode/**` so documentation and tool configuration never trigger a build.

- **Docs-only exception:** when a commit changes only paths that `ci.yml`'s `paths-ignore` covers — `**.md`, `project-docs/**`, `LICENSE`, `.claude/**`, `.vscode/**` — skip steps 1–5 and 8: stage and commit on the current branch, then push (step 9). The filter stops the run, so the push costs nothing and origin keeps the only offsite copy of the work. Check the whole push before you trust it: `git diff --name-only origin/run-seamlyLayout..HEAD` lists every file the push carries, and one unmatched path starts the full suite. **`.txt` and `.svg` are NOT docs for this purpose** — `CMakeLists.txt` is a build input and an `.svg` can be a compiled Qt resource, so a commit touching either needs the skip token or needs the run
