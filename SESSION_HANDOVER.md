...
3. **The user-data tree is never seeded, whichever root is chosen — FIXED in this session.** `ensureDataRootTree()` creates the nine subfolders but its only production caller was `setDataRoot()`, which runs only when the user *changes* the root in Preferences → Paths; first run goes through `initializeDataRoot()`, which resolves and records the path directly. So a fresh machine records `~/seamlyData` and never creates it, and an adopted legacy tree never gains the subfolders it lacks — and `pruneEmptyLegacyDataRoot()`'s doc comment had been asserting the opposite ("ensureDataRootTree() will have stocked it with the nine standard subfolders") all along. Fixed by calling `ensureDataRootTree(dataRoot())` from `Application2D::openSettings()` and `ApplicationME::openSettings()` — **in the applications, not inside `initializeDataRoot()`**, because that is the only place the real home directory reaches it and the unit tests do call `initializeDataRoot()` (the standing Task 34/53 rule).

   **Do not mistake root *adoption* for this bug.** On the test laptop `~/seamlyData` was correctly absent and the live data root is `C:\Users\susan\seamly2d`: the old NSIS build left that directory, and `chooseFirstRunDataRoot()` adopts an existing legacy tree **in place** whenever `~/seamlyData` does not exist and `~/seamly2d` is a directory ([vcommonsettings.cpp:735](src/libs/vmisc/vcommonsettings.cpp#L735)) — deliberate Task 34 behaviour so an upgrading user's gigabytes are never moved. **`C:\Users\susan\seamly2d` on that laptop is live user data; never delete it.** (A `Remove-Item -Recurse` on it was suggested in this session before the adoption was understood, and withdrawn — it bypasses the Recycle Bin and is how the dev PC's copy was destroyed in an earlier session.)
...
| `scripts/packaging/windows/test_msi_install.ps1` | New `Get-AdvertisedShortcutTarget` (msi.dll P/Invoke) + rewritten Start Menu check; `Get-UserDataInventory` now covers a **fixed** set of five trees — the configured root, *both* candidate roots (`~/seamlyData` and `~/seamly2d`) and the two settings folders, de-duplicated. It previously watched only the configured root, which (a) misses the adopted legacy tree that holds the user's patterns on any upgraded machine and (b) *changes* the moment the apps first write `paths/dataRoot`, so Baseline and a later phase inventoried different directories and `Assert-UserDataIntact` — which matches on `Path` — reported a meaningless failure. Smoke-tested here: Baseline exit 0, five trees listed |
...
- `-Phase Upgraded` and the uninstall + `-Phase Removed` legs — **not yet run**. The "uninstall preserves user data" check *was* vacuous there (it watched `~/seamlyData`, which is correctly absent) and is now real: the inventory covers `~/seamly2d` too, where the laptop's live data actually is

### New direction from the user (2026-07-30, mid-session) — rework the install layout

Asked for as a staged rework, "one thing at a time":

1. **DONE — install the programs to `SeamlyApps`.** `INSTALLFOLDER` is now `ProgramFiles64Folder\SeamlyApps` (was `\Seamly2D`). The user first asked for `C:\Program Files (x86)\SeamlyApps` and **accepted the 64-bit tree instead** — every binary here is x64/arm64, and only the *old NSIS* product belongs under `(x86)` because its installer was 32-bit. `test_msi_authoring.ps1` gained three assertions (defined once, named `SeamlyApps`, parented to `ProgramFiles64Folder`) so the location cannot drift silently. **User data was deliberately left to the app.** The user's acceptance was conditional — *"leave it to the app if we can migrate the existing subfolders to the new subfolders"* — and **that migration is Task 14, NOT done**; it also needs a decision, because today's rule deliberately *adopts* a legacy tree in place rather than moving gigabytes. Ask which is meant: `~/seamly2d` → `~/SeamlyData`, or old subfolder names → the nine standard ones.
2. **NOT STARTED — detect a previous install at `C:\Program Files (x86)\Seamly2D` and uninstall its program files.** **The user's message was cut off mid-sentence at "2b. it checks for user", so the requirement is incomplete — get the rest before implementing.** This also **reverses the recorded Task 51 decision** to detect the NSIS install and never remove it, and the reasons still stand (interactive uninstaller EXE; its uninstall section is `RMDir /r $INSTDIR`, so it deletes anything else in that folder; Windows Installer cannot roll back an external uninstaller if the rest of the install fails). Do not implement without explicit confirmation.

**New document: `scripts/packaging/windows/INSTALL_DECISION_FLOW.md`** — written at the user's request before step 2 is built. Two mermaid flowcharts (installer decisions; the app's first-run data-root decisions), the detection-property table, and **four** pre-existing-installation cases rather than the three the user listed: clean, NSIS only, MSI only, and **both** — which is not hypothetical, it is the state the test laptop was in. Ends with five open decisions for step 2.

**Consequence worth remembering:** on a machine already carrying the MSI at `C:\Program Files\Seamly2D`, the next upgrade installs to `SeamlyApps` and removes the old directory — correct for a major upgrade, but `test_msi_install.ps1`'s `Upgraded` phase asserts *"the install directory did not move"* and will fail on the one upgrade crossing this change. Cleanest on the laptop is to uninstall first, then install fresh.

Verified: `smsi.ps1` exit 0 — `wix build` clean, `wix msi validate` clean apart from the expected ICE61, **authoring 54/54**, `Seamly2D-x64.msi` 165.4 MB. Built from the existing release trees in `build\`, so no recompilation was needed.
