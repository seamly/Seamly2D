...
- [ ] Rename.2.13 Verify the build and the four test binaries through `ci.yml` — the local build and test scripts were deleted in August 2026. If you do build by hand, wipe the shadow-build tree first; a stale `Makefile`/object tree can link an old object and mask a missed include (Task 46)
- [ ] Rename.2.14 Confirm CI stays green on `ci.yml`, the one workflow that compiles these sources
