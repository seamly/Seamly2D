...
1. Ship `packaging/licenses/qt_notices.txt`: LGPL-3.0 statement, Qt source link,
   Qt modules and their third-party components, all license texts.
2. Not statically link Qt in a way that would require relicensing under GPL.

`qt_notices.txt` is committed and regenerated by
`packaging/licenses/generate_license_notices.py`. See `.github/README-BUILDS.md`,
