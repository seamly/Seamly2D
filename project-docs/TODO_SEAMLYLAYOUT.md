...
- [x] Layout.8.2 Determine what sets `input_directory`/`layout_directory` to `<DataRoot>\layouts` at runtime, and why `preferences_directory`/`settings_directory` have no equivalent correction.
  Resolved 2026-08-30: the tester's `<DataRoot>\layouts` observation was the *intended* design (project owner confirmed: one shared `layouts` folder for both import and export, directly under the data root, matching the `layouts` subfolder Seamly2D's `ensureDataRootTree()` already creates under `%DATAROOT%`) — the code just did not implement it yet (it produced separate `${HOME}/seamlyLayout/input` and `/output`). No runtime code was ever setting it; the observed value was aspirational. Fixed directly: `default_preferences.json`'s `input_directory`/`layout_directory` (all three platform blocks) and their matching hardcoded fallbacks in `seedFromBundledDefaults()` now both read `${HOME}/layouts` (first landed as `${HOME}/seamlyLayout/layouts`, corrected the same session to drop the extra `seamlyLayout` nesting level, per the project owner pointing at the live seeded value), so `expandDefaultPathTokens()` still applies its existing DataRoot substitution unchanged — no code path change, only the template string. The pre-existing static-analysis findings (no QML/Rust code sets either field to a `layouts` leaf; the MSI ships no `settings/preferences.json` for `migrateLegacyPreferencesJson()` to pick up) stand and are superseded by this resolution.
