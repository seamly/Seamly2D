...
- [ ] Seamly2D.2.1 `MainWindow::Open()` (`src/app/seamly2d/mainwindow.cpp:4378-4408`) ignores `VSettings::getPatternPath()`. It always opens the dialog in the last-opened file's folder, or `QDir::homePath()` if the recent-files list is empty. Add a fallback to `qApp->Seamly2DSettings()->getPatternPath()` when no usable recent-file directory exists. CONFIRMED live 2026-09-01 (test doc B.2a-iii, fresh install): the dialog opened in `C:\Users\<user>` even though `qt6_seamly2d.ini` held `pattern=C:/Users/susan/Documents/SeamlyData/patterns` — the empty-recent-files → `QDir::homePath()` branch.
