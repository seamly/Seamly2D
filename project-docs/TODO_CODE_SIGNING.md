...
- [ ] CodeSign.1.7 (While NSIS is still the released Windows installer) apply the same inner-exe signing to `ci.yml`'s NSIS installer so its bundled apps are signed too, until the MSI replaces it — or explicitly defer if NSIS is being retired first
