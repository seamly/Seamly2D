...
- [ ] InstMacOS.2.4 Reflect the "Seamly" suite umbrella in the macOS packaging where user-visible (dmg/app naming as appropriate) without breaking the bundle identifiers set in Task 16
...
- [ ] InstMacOS.2.7 Detect a pre-existing **standalone** Seamly2D/SeamlyMe install that the suite MSI's `UpgradeCode`/`MajorUpgrade` does not already supersede — the NSIS installer's ARP/uninstall registry entry and/or an install under `%ProgramFiles(x86)%\Seamly2D` — during the suite MSI's install sequence (WiX `<Upgrade>`/`FindRelatedProducts` on the old product code, or a registry/`AppSearch` probe) — 1c
- [ ] InstMacOS.2.8 Only when **SeamlyLayout is not installed** (i.e. this is an upgrade from the pre-suite standalone apps, not a repair/upgrade of a suite install): run the old uninstaller / remove the old product before laying down the suite payload, so the two do not coexist — 1c
...
- [ ] InstMacOS.2.12 Verify the reported scenario end-to-end: on a machine with the old `%ProgramFiles(x86)%\Seamly2D` standalone install and no SeamlyLayout, running the suite MSI removes the old install, installs the suite once (no duplicate), and leaves all pre-existing user files/patterns/measurements intact
- [ ] InstMacOS.2.13 Document the migrate-from-standalone behavior and the no-overwrite guarantee in `packaging/windows/README.md` and `.github/README-BUILDS.md`
